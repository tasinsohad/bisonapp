...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row bg-[#ffffff] font-sans antialiased overflow-hidden">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 md:p-12 bg-slate-900 relative overflow-hidden group/bg">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          {/* Main Orbs */}
          <div 
            className="absolute top-[10%] left-[20%] w-[450px] h-[450px] bg-blue-500 rounded-full filter blur-[120px] opacity-30 animate-pulse transition-transform duration-300 ease-out" 
            style={{ 
          ...
          ></div>
          <div 
            className="absolute bottom-[10%] right-[10%] w-[350px] h-[350px] bg-indigo-500 rounded-full filter blur-[100px] opacity-20 animate-pulse transition-transform duration-300 ease-out" 
            style={{ 
          ...
          <div 
            className="absolute w-[300px] h-[300px] bg-purple-500/20 rounded-full filter blur-[80px] transition-transform duration-75 ease-out pointer-events-none opacity-0 group-hover/bg:opacity-100 mix-blend-screen"
            style={{
              left: '50%',
              top: '50%',
        ...
        <div 
          className="w-full max-w-md p-8 md:p-10 rounded-[32px] bg-white/5 border border-white/10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] backdrop-blur-2xl relative z-10 animate-fade-in flex flex-col items-center hover:shadow-[0_40px_100px_-16px_rgba(59,130,246,0.15)] hover:bg-white/10 transition-all duration-300 ease-out group/card"
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-transparent opacity-0 group-hover/card:opacity-100 group-hover/card:translate-x-full transition-all duration-1000 pointer-events-none rounded-[32px]"></div>

          <div className="text-center w-full mb-8 relative z-10" style={{ transform: 'translateZ(30px)' }}>
            <h2 className="text-3xl font-bold text-white mb-1.5 transition-colors">{isSignUp ? 'Join Us!' : 'Welcome Back'}</h2>
            <p className="text-slate-300 text-sm font-medium">{isSignUp ? 'Create a new account to get started.' : 'We are really happy to see you again!'}</p>
          </div>

          {error && (
            <div className="w-full mb-6 p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-200 text-sm text-center font-medium animate-shake relative z-10">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleAuth} className="w-full space-y-5 relative z-10" style={{ transform: 'translateZ(40px)' }}>
            <div className="group">
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-blue-400 transition-colors z-20">
                  <Mail className="h-5 w-5" />
                </div>
                {/* Input Glow Effect */}
                <div className="absolute inset-[-2px] bg-gradient-to-r from-blue-500 to-indigo-500 rounded-2xl opacity-0 group-focus-within:opacity-100 blur-sm transition-opacity duration-300"></div>
                <input
                  type="email"
                  required
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="block w-full pl-11 pr-4 py-3.5 border border-white/10 rounded-2xl bg-white/5 text-white placeholder-slate-400 focus:outline-none focus:bg-white/10 transition-all text-sm font-medium backdrop-blur-md relative z-10"
                  placeholder="Email"
                />
              </div>
            </div>
            ...
            <div className="group">
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-11 pr-11 py-3.5 border border-white/10 rounded-2xl bg-white/5 text-white placeholder-slate-400 focus:outline-none focus:bg-white/10 transition-all text-sm font-medium backdrop-blur-md relative z-10"
                  placeholder="Password"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-200 transition-colors z-20"
                >
            ...
            >
              ...
              <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-in-out pointer-events-none"></div>
          ...
          <div className="mt-8 text-center relative z-10" style={{ transform: 'translateZ(20px)' }}>
            <button
              ...
              className="text-xs text-slate-400 hover:text-white transition-colors font-semibold px-4 py-2 hover:bg-white/10 rounded-lg"
            >
